...
struct DGLineChartRepresentable: NSViewRepresentable {
    ...
    func updateAccount () {
        ...
        let allTransactions = ListTransactionsManager.shared.getAllData(from: nil, to: nil, ascending: true)
        print("updateAccount — total:", allTransactions.count, "selectedStart:", self.viewModel.selectedStart, "selectedEnd:", self.viewModel.selectedEnd)

        let sortedTransactions = allTransactions.sorted { $0.datePointage < $1.datePointage }
...
//        allTransactions.sort { $0.datePointage < $1.datePointage }
        ...
        let firstPointageStart = calendar.startOfDay(for: sortedTransactions.first!.datePointage)
        let lastPointageStart  = calendar.startOfDay(for: sortedTransactions.last!.datePointage)
        ...
        print("updateAccount — range:", startDate, "->", endDateExclusive)
        ...
        print("updateAccount — filtered count:", filteredTransactions.count)
...
//            self.parent.viewModel.startEmpty = false
        ...
        {
            print("chartValueNothingSelected — clearing selection and unlocking day selection")
            DispatchQueue.main.async {
                var didChange = false
